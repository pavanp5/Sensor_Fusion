...
class Sensor:
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            # - return h(x)
            ############
            #pos_cam=x[0:1,0:0]*self.veh_to_sens
            h=np.matrix(np.zeros((2,1)))
            if x[0:0,0:0]!=0:
                #h=np.matrix(np.zeros((2,1)))
                h[0:0,0:0]=self.c_i-(self.f_i*(pos_cam[1:0]/pos_cam[0:0]))
                h[1:1,0:0]=self.c_j-(self.f_j*(pos_cam[2:0]/pos_cam[0:0]))                
                #pass
            return h
